...
| 表格 / Git / JSON·YAML / Word / 股票 | **`csv_tsv_summary`**、**`xlsx_sample`**、**`git_snapshot`**、**`structured_pick`**、**`docx_to_text`**、**`stock_quote`/`stock_kline`/`stock_history_trades`/`stock_kline_summary`** | 已实现 | P0 | `.omlxcli/skills/spreadsheet.py`、`git_readonly.py`、`structured_data.py`、`docx_read.py`、`stock_market.py` | 股票技能含东财风控检测+内置解封脚本+新浪兜底；依赖见 `requirements.txt`（含 `opencv-python` 等） |
